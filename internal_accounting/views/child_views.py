...
from internal_accounting.forms import ChildStudentForm, ChildRefundForm
...
def child_student_list(request):
    current_year = request.GET.get('year', str(timezone.now().year))
    all_years = Child.objects.values_list('statement_year', flat=True).distinct().order_by('-statement_year')
    ...
    if any([
        current_status, current_desired_service,
    ...
    return render(request, 'internal_accounting/child/student_list.html', context)


def child_student_detail(request, pk):
    student = get_object_or_404(Child, pk=pk)
    refunds = ChildRefund.objects.filter(student=student).order_by('-date_of_refund')
    ...
    return render(request, 'internal_accounting/child/student_detail.html', context)


def child_student_visits(request, pk):
    student = get_object_or_404(Child, pk=pk)
    context = {'student': student}
    return render(request, 'internal_accounting/child/student_visits.html', context)


def child_add_student(request):
    if request.method == 'POST':
        form = ChildStudentForm(request.POST)
        if form.is_valid():
            student = form.save(commit=False)
            student._imported = False
            student.save()
            messages.success(request, f'Студент {student.fio} успешно добавлен!')
            return redirect('internal_accounting:child:student_list')
    else:
        form = ChildStudentForm()
    
    context = {
        'form': form,
        'title': 'Добавить детского студента',
    }
    
    return render(request, 'internal_accounting/child/student_form.html', context)


def child_edit_student(request, pk):
    student = get_object_or_404(Child, pk=pk)
    
    if request.method == 'POST':
        form = ChildStudentForm(request.POST, instance=student)
        if form.is_valid():
            form.save()
            messages.success(request, f'Данные студента {student.fio} обновлены!')
            return redirect('internal_accounting:child:student_detail', pk=pk)
    else:
        form = ChildStudentForm(instance=student)
...
def child_delete_student(request, pk):
    student = get_object_or_404(Child, pk=pk)
    ...
    context = {'student': student}
    return render(request, 'internal_accounting/child/student_confirm_delete.html', context)


def child_refund_add(request):
    ...
    if request.method == 'POST':
        form = ChildRefundForm(request.POST)
        if form.is_valid():
            refund = form.save(commit=False)
            refund.save()
            messages.success(request, f'Возврат средств на сумму {refund.summ_of_refund} зарегистрирован!')
            return redirect('internal_accounting:child:student_detail', pk=refund.student_id)
    else:
        form = ChildRefundForm()
        if student:
    ...
    return render(request, 'internal_accounting/child/refund_form.html', context)
...
def child_export_excel(request):
    ...
    headers = [
        '№', 'ФИО', 'Родитель', 'Курс', 'Дата покупки', 'Телефон',
        'Сумма платежа', 'Всего часов', 'Откатано', 'Остаток',
    ...
    filename = f'child_students_{timezone.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
...
def child_import_students(request):
    if request.method == 'POST':
        excel_file = request.FILES.get('excel_file')
        if not excel_file:
            messages.error(request, 'Файл не загружен!')
            return redirect('internal_accounting:child:import_students')
        
        # Логика импорта
        messages.success(request, 'Студенты успешно импортированы!')
        return redirect('internal_accounting:child:student_list')
    
    return render(request, 'internal_accounting/child/import_students.html')